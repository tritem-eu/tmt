...
describe Oslc::Qm::TestPlan::Resource do
  ...
  def doc(parser)
    content = ''
    plan = ::Oslc::Qm::TestPlan::Resource.new(test_run.reload)
    if parser == :xml
      content = plan.to_xml
    elsif parser == :rdf
      content = plan.to_rdf
  ...
  [:xml, :rdf].each do |format|
    describe "for '#{format.to_s.upcase}' format" do
      ...
      it "should show 'rdf:about' property" do
        ...
        result.should eq server_url("/oslc/qm/service-providers/#{project.id}/test-plans/#{test_run.id}")
      end

      it "should show 'rdf:type' property" do
        doc(format).to_xml.should include("TestPlan")
      end

      it "shouldn't show 'dcterms:contributor' property" do
        test_run.project.members.delete_all
        test_run.project.members.should be_empty
        doc(format).xpath("//dcterms:contributor").should be_empty
      end

      it 'shows "contributor" property' do
        ready(project)
        test_run.project.members.should_not be_empty
      ...
      it "should show 'dcterms:title' property" do
        doc(format).xpath('//dcterms:title').text.should eq(test_run.name)
      end

      it "should show 'dcterms:created' property" do
        time = Support::Oslc::Date.new(test_run.created_at).iso8601
      ...
      it "should show 'dcterms:description' property" do
        doc(format).xpath('//dcterms:description').text.should eq(test_run.description)
      end

      it "should show 'oslc:instanceShape' property" do
        result = doc(format).xpath('//oslc:instanceShape').first.attributes['resource'].value
        result.should eq server_url("/oslc/qm/resource-shapes/test-plan")
      end

      it "should show 'oslc_qm:usesTestCase' property" do
        ready(project)
        result = doc(format).xpath('//oslc_qm:usesTestCase').first.attributes['resource'].value
      ...
      it "should show 'dcterms:identifier' property" do
        plan_url = server_url("/oslc/qm/service-providers/#{project.id}/test-plans/#{test_run.id}")
